#!/usr/bin/env bash
# Produces the LGPL 6(a) source offer for the pinned WebKit tag
# (docs/licensing.md section 6): webkit-source-<tag>.tar.zst plus SHA256SUMS.
#
#   ./scripts/source-offer.sh
#   ./scripts/source-offer.sh --dry-run
#   ./scripts/source-offer.sh --out dist-source --release v0.2.0
#
# The tarball is `git archive` of the pinned tag, never a plain `tar` of the
# checkout directory: the developer checkout also holds WebKitBuild/ (tens of
# GB of build products) and .git, and neither of those is source. Archiving
# the tagged commit is also deterministic -- same tag, same bytes -- which is
# what lets the release workflows cache the tarball keyed on the tag.
#
...
TARBALL="$OUT_DIR/webkit-source-$WEBKIT_TAG.tar.zst"
SUMS="$OUT_DIR/SHA256SUMS"

# Resolve the tree to archive. `git archive` packs the tagged commit, so
# untracked build products (WebKitBuild/) and working-tree dirt cannot leak
# into the tarball; what has to be proven is that HEAD *is* the pinned tag.
SRC=""
SRC_DESC=""
...
Move it aside or set ZER0_WEBKIT_DIR to a different directory."
	...
	if [[ "$at" == "$WEBKIT_TAG" ]]; then
		SRC="$WEBKIT_SRC"
		SRC_DESC="existing checkout at $WEBKIT_SRC"
	else
		die "$WEBKIT_SRC is at '${at:-nothing describable}', not the pinned $WEBKIT_TAG.
Re-run ./scripts/webkit/fetch.sh (idempotent) or point ZER0_WEBKIT_DIR elsewhere.
An offer published under a tag must come from that tag."
	fi
fi
...
if ((DRY_RUN)); then
	...
	echo "    tag:     $WEBKIT_TAG"
	if [[ -n "$SRC" ]]; then
		echo "    source:  $SRC_DESC (verified at the pinned tag)"
	else
		echo "    source:  shallow clone of $WEBKIT_UPSTREAM at $WEBKIT_TAG"
	...
	echo "    plan:    git archive --format=tar --prefix=WebKit/ $WEBKIT_TAG"
...
if [[ -z "$SRC" ]]; then
	...
	note "no checkout at $WEBKIT_SRC, cloning $WEBKIT_TAG (shallow)"
	git clone --depth 1 --branch "$WEBKIT_TAG" "$WEBKIT_UPSTREAM" "$CLONE_DIR"
	SRC="$CLONE_DIR"
...
mkdir -p "$OUT_DIR"
if [[ -f "$TARBALL" ]]; then
	note "$(basename "$TARBALL") already exists, reusing (git archive output is deterministic per tag)"
else
	note "archiving $WEBKIT_TAG -> $(basename "$TARBALL") (zstd -19; this is the slow part)"
	# Written as .part so a failed pipe cannot leave a corrupt tarball that a
	# later run would mistake for the finished offer.
	git -C "$SRC" archive --format=tar --prefix=WebKit/ "$WEBKIT_TAG" |
