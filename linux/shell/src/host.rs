...
fn perform(app: &Rc<App>, command: EngineCommand) {
    ...
    match command {
        EngineCommand::CreateWebView {
            ...
            view.connect_load_changed(clone!(@weak app => @default-return (), move |view, event| {
            ...
            view.connect_estimated_load_progress_notify(
                clone!(@weak app => @default-return (), move |_view| {
                    let app = app.clone();
            ...
            view.connect_notify_local(
                "title",
                clone!(@weak app => @default-return (), move |view, _| {
                    if let Some(title) = view.title() {
                        run(&app, Action::TitleChanged { tab, title: title.to_string() });
                    }
                }),
            );
...
fn sync_chrome(app: &Rc<App>) {
    ...
    for (index, tab) in tabs.iter().enumerate() {
        ...
        click.set_button(2);
        click.connect_pressed(clone!(@weak app => move |_, _, _, _| {
    ...
    new_tab.connect_clicked(clone!(@weak app => move |_| {
...
pub fn app_for(
    ...
    load_bar.set_draw_func(
        clone!(@strong progress_for_draw, @strong bar_color => @default-return (), move |_area, cr, width, height| {
            let fraction = progress_for_draw.get();
            if fraction <= 0.0 || width <= 0 {
                return;
            }
            cr.set_source_rgba(bar_color.red(), bar_color.green(), bar_color.blue(), bar_color.alpha());
    ...
    back.connect_clicked(clone!(@weak app => move |_| {
        run_ui_command(&app, UiCommand::Back);
    }));
    forward.connect_clicked(clone!(@weak app => move |_| {
        run_ui_command(&app, UiCommand::Forward);
    }));
    reload.connect_clicked(clone!(@weak app => move |_| {
        run_ui_command(&app, UiCommand::Reload);
    }));
    new_tab_action.connect_clicked(clone!(@weak app => move |_| {
    ...
    entry.connect_activate(clone!(@weak app => move |entry| {
        let tab = app.host.borrow().session.browser.active_tab();
        if let Some(tab) = tab {
            run(&app, Action::NavigateTo {
                tab,
    ...
    keys.connect_key_pressed(clone!(@weak app => @default-return gtk::glib::Propagation::Proceed, move |_, keyval, _, state| {
        key_pressed(&app, keyval, state)
    }));
